/**
 * components/columns/PostCard.jsx — 수정: ReactionBar 추가
 */
...
export default function PostCard({ boardId, colId, post, onOpen, reactionType = 'none' }) {
  const attachCount = (post.attachments ?? []).length
  return (
    <article className="post-card" onClick={() => onOpen?.(post)} style={{ cursor:'pointer' }}>
      <div className="post-card-title">{post.title}</div>
      {post.content && (
        <div className="post-card-content"><LinkifiedText text={post.content} /></div>
      )}
      {attachCount > 0 && (
      ...
      <ReactionBar boardId={boardId} itemId={post.id} itemType="post" reactionType={reactionType} />
    </article>
  )
}
