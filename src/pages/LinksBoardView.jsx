    ...
    <div>
      ...
      <div className="board-detail-header">
        ...
        <div className="board-detail-actions">
          ...
          <Button variant="primary" size="sm" onClick={linkModal.open}>
            ＋ 링크 추가
          </Button>
        </div>
      </div>
      ...
      {filtered.length === 0 ? (
        <EmptyState
          ...
          onAction={linkModal.open}
        />
      ) : viewMode === 'grid' ? (
        <div className="links-grid">
          {filtered.map((link) => (
            <LinkCard key={link.id} boardId={board.id} link={link} viewMode="grid" />
          ))}
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {filtered.map((link) => (
            <LinkCard key={link.id} boardId={board.id} link={link} viewMode="list" />
          ))}
        </div>
      )}
